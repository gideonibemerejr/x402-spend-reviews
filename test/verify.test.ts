...
test("a matching transfer verifies", async () => {
  ...
  expect(await verifySettlement(submission(), rpc)).toEqual({ verified: true, proof: PROOF.paymentTraced, amount: "10000" });
});
...
test("the payer is read from the transfer log, not the transaction sender", async () => {
  ...
  expect(await verifySettlement(submission({ payer: PAYER }), rpc)).toEqual({ verified: true, proof: PROOF.paymentTraced, amount: "10000" });
...
test("an explicit asset address overrides the symbolic USDC lookup", async () => {
  ...
  expect(await verifySettlement(submission({ asset: token }), rpc)).toEqual({ verified: true, proof: PROOF.paymentTraced, amount: "10000" });
});
...
test("addresses compare case-insensitively", async () => {
  ...
  expect(result).toEqual({ verified: true, proof: PROOF.paymentTraced, amount: "10000" });
});
